...
options=("Download_all" "Categories" "Quit")
...
do
 case $opt in
  ...
  "Categories")
   echo " "
   PS3=`echo -e "\n\t[+] Please enter your choice: \n"`
   options=("auth" "broadcast" "brute_force" "default" "discover" "dos" "exploit" "external" "fuzzer" "intrusive" "malware" "safe" "version" "vuln" "Quit")
   select opt in "${options[@]}"
   do
       case $opt in
           "auth")
